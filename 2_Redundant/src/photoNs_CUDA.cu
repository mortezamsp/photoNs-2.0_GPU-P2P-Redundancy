...
int allocMemGPU(int procSize, int maxPartsInLeaf, int MAXTASK, int verbosity_gpu)
{
    cudaError_t e;
    ...
    if(d_pos_data == NULL)
    {
        ...
        e = cudaMalloc((void**)&d_pos_data, procSize * 2 * max_res_pos_size_ * sizeof(double)); //why 2 ? to prevent several reallocations and memory fragmentation
        if(e != cudaSuccess)
        {
            //if(verbosity_gpu) 
            printf("Error allocating memory for d_pos_data : ErrorCode = %d, %s\n\t\tres_pos_size_ = %d\n", 
                                       e, cudaGetErrorString(e), max_res_pos_size_*2*procSize);
    ...
    if(d_acc_data == NULL)
    {
        ...
        e = cudaMalloc((void**)&d_acc_data, procSize * max_acc_size * 2 * sizeof(double));
        if(e != cudaSuccess)
        {
            //if(verbosity_gpu) 
        ...
        if(verbosity_gpu) printf("memory allocated to d_acc_data %1.f MB\n", (double)max_acc_size*procSize*8*2/1024.0/1024.0 );
    }
    // //memset
    // if(verbosity_gpu) printf(">> \tmemsetting d_acc_data ...\n");
    // e = cudaMemset(d_acc_data, 0, max_acc_size * procSize * sizeof(double));
    // if(e != cudaSuccess)
    // {
    //     //if(verbosity_gpu) 
    //     printf("\nError memeset for d_acc_data : %s\n\t\trequested size = %1.f MB\n",
    //                                  cudaGetErrorString(e), ((double)max_acc_size * 8 * procSize * 2 * sizeof(double))/1024./1024.0);
    //     getGPUMemoryState(verbosity_gpu);
    //     return -1;
    // }
    ...
    if(d_pos_index == NULL)
    {
        ...
        e = cudaMalloc((void**)&d_pos_index, max_posIndexSize*procSize*2*sizeof(int));
        if(e != cudaSuccess)
        {
            //if(verbosity_gpu) 
            printf("Error allocating memory for d_pos_index : %s\n\t\t4posSize = %d\n", 
                                        cudaGetErrorString(e), max_posIndexSize*procSize);
            getGPUMemoryState(verbosity_gpu);
            return -1;
        }
...
int copyMemGPU(
    ...
    int procSize, int procRank, int maxPartsInLeaf, int numtasks, int posCounter, int verbosity_gpu)
{
    cudaError_t e;
    ...
    e = cudaMemcpy(d_pos_data+procRank*max_res_pos_size_, h_pos_data[procRank], posCounter * sizeof(double), cudaMemcpyHostToDevice);
    if(e != cudaSuccess)
    {
        //if(verbosity_gpu) 
        ...
        printf("\n\th_pos_data[0] = %f, res_pos_size_ = %d, d_pos_data = %s\n",
                                    h_pos_data[procRank][0], posCounter, d_pos_data==nullptr?"null":"not null");
        return -1;
    }

    //copy indexing array
    if(verbosity_gpu) printf(">> \tcopying d_pos_index ...\n");
    e = cudaMemcpy(d_pos_index+ procRank*max_posIndexSize, h_pos_index[procRank], posIndexSize*sizeof(int), cudaMemcpyHostToDevice);
    if(e != cudaSuccess)
    {
        //if(verbosity_gpu) 
...
void readResultsGPU(double** h_acc_data, int procRank, int procSize, int maxPartsInLeaf, int MAXTASK, int partCounter, int verbosity_gpu)
{
    int max_acc_size = maxPartsInLeaf * MAXTASK * 3;
    int accSize = partCounter;
    cudaError_t e = cudaMemcpy(
            h_acc_data[procRank], 
            d_acc_data+procSize*max_acc_size, accSize * sizeof(double), cudaMemcpyDeviceToHost);
    if(e != cudaSuccess)
    {
        //if(verbosity_gpu) 
...
extern "C" {
    int LaunchKernelP2PDualNaive(int procSize, int PROC_RAKN, int nTasks, double SoftenScale, double MASSPART, int verbosity_gpu) 
    {
