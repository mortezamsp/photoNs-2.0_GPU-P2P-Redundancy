...
void *task_compute_p2p_ext(void *argt_ex ) {
	...
	for (n=0; n<nt; n++) 
	{
		...
		//store indices
		h_pos_index[PROC_RANK][n*5 + 0] = posCounter; //start index of data for this parital interaction
		h_pos_index[PROC_RANK][n*5 + 1] = im; //im - first_leaf; //target leaf
		h_pos_index[PROC_RANK][n*5 + 2] = leaf[im].npart; //num target particles
		h_pos_index[PROC_RANK][n*5 + 3] = exrtree[jm].npart; //num source particles
		//h_pos_index[PROC_RANK][n*5 + 4] = partCounter; //start index of results for this leaf
		h_pos_index[PROC_RANK][n*5 + 4] = n * maxPartsInLeaf * 3;
		//partCounter += (leaf[im].npart * 3);
		
		if(leaf[im].npart <= 0 || exrtree[jm].npart <= 0)
		{
			h_pos_index[PROC_RANK][n*5 + 2] = 0;
			h_pos_index[PROC_RANK][n*5 + 3] = 0;
			continue;
		}
		//partCounter += (leaf[im].npart * 3);
		...
		{
			h_pos_data[PROC_RANK][posCounter++] = part[ip].pos[0];
			h_pos_data[PROC_RANK][posCounter++] = part[ip].pos[1];
			h_pos_data[PROC_RANK][posCounter++] = part[ip].pos[2];
		}

		//collect sources
		for (int jp=jstart; jp<jend; jp++)
		{
			h_pos_data[PROC_RANK][posCounter++] = exrbody[jp].pos[0];
			h_pos_data[PROC_RANK][posCounter++] = exrbody[jp].pos[1];
			h_pos_data[PROC_RANK][posCounter++] = exrbody[jp].pos[2];
		}
	}
	...
	{
		//int im = task_t_ex[c][n];
		int im = h_pos_index[PROC_RANK][n*5 + 1]; 
		//if(leaf[im].npart <= 0)
		if(h_pos_index[PROC_RANK][n*5 + 2] == 0 || h_pos_index[PROC_RANK][n*5 + 3] == 0)
			continue;
		partOffset = n * maxPartsInLeaf * 3;
		partCounter = 0;
		for (int ip=leaf[im].ipart; ip<leaf[im].ipart+leaf[im].npart; ip++)
		{
			part[ip].acc[0] = h_acc_data[PROC_RANK][partOffset + partCounter * 3 + 0];
			part[ip].acc[1] = h_acc_data[PROC_RANK][partOffset + partCounter * 3 + 1];
			part[ip].acc[2] = h_acc_data[PROC_RANK][partOffset + partCounter * 3 + 2];
			partCounter++;
		}
	}
