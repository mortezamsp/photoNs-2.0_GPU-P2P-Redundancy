...
void initGPUarrays()
{
	...
	if(!arraysInitialized)
	{
		...
		resultSize = MaxLenTask * resultChunk * upperBound; //results are duplicated first, and i must integrate it in CPU after kernel called
		...
		interactionSize = MaxLenTask * interactionChunk * upperBound;
...
void fmm_task() {
	int n;
	double t1  = dtime();
	LEN_TASK = 163834;//NLEAF * maxNeighbors; //16384;
